...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($GenerarReporte)) {
        ...
        $query = "SELECT cc.id_control AS id, tg.descripcion AS tipoGasolina, e.descripcion as eco, u.descripcion AS ubicacion, cc.kilometraje as kilometraje, 
        ...
        JOIN eco e ON cc.id_eco = e.id_eco JOIN ubicacion u ON cc.id_ubicacion = u.id_ubicacion 
        JOIN resguardatarios resg ON cc.resguardatario = resg.id_resguardatario JOIN region reg ON resg.region = reg.id_region AND cc.fecha='$dia' ORDER BY(id) ASC";
        ...
        $pdf->SetTitle('Reporte de Combutible');
        $pdf->Cell(90);
        ...
        $query2 = "SELECT fecha as fecha2, SUM(litros_gasolina) as total_litros, SUM(costo) AS monto_total
        FROM control_combustible WHERE fecha='$dia'";
        $datos2 = $obj->obtenerDatos($query2);

        foreach ($datos2 as $row2) {
            $pdf->Cell(50, 10, $row2['fecha2'], 1, 0, 'C');
            $pdf->Cell(50, 10, $row2['total_litros'], 1, 0, 'C');
            $pdf->Cell(50, 10, '$ ' . $row2['monto_total'], 1, 0, 'C');
        }

        $fechaActual = date('d-m-Y');
        $pdf->Output('Reporte de Combustible ' . $dia . '.pdf' . $fechaActual, 'D');
        // $pdf->Output();
    }
}
